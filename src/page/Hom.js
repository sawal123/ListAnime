...
function Home() {
  ...
  if (todo.length > 0) {
    return (
      <div >
        <div className='my-4'>
          <input type='text' value={inputValue} onChange={handleInputChange} className='border-2 lg:w-50  rounded-lg p-2' placeholder='Cari..' />
        </div>
        <div className='grid grid-cols-2 lg:grid-cols-4  gap-2'>
          {
            (inputValue ? filteredTodo : todo).map((item) => (
              <div key={item.mal_id} className='border-1 rounded-lg bg-slate-200 p-3'>
                <img className='rounded-lg' alt='' style={{ objectFit: 'cover', width: "100%", height: "300px" }} src={item.images.jpg.image_url} />
                <div className='mt-3'>
